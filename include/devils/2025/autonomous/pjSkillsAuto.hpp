...
namespace devils
{
    struct PJSkillsAuto
    {
        ...
        {
            // PID Params
            PIDController::Options rotatePID = {0.7, 0.0, 60.0};
            ...
            // Ring 3
            pjRoutine.driveToTrajectory(28, -48, 0, false, 0, 2)->run();
            intakeOnce(conveyor, pjRoutine);

            // Ring 4
            pjRoutine.rotateTo(180)->run();
            pjRoutine.driveToTrajectory(12, -48, 180, false, 12, 12)->run();
            pjRoutine.driveToTrajectory(-24, -24, 180, false, 12, 8)->run();
            pjRoutine.driveToTrajectory(-28, -20, 180, false, 0, 1)->run();
            intakeOnce(conveyor, pjRoutine);

            // Corner
            pjRoutine.driveToTrajectory(-52, -51, 225, false, 0, 12, slowConstraints)->run();
            pjRoutine.driveToTrajectory(-65, -64, 225, false, 0, 12, fastConstraints)->run();
            intakeOnce(conveyor, pjRoutine);
            ...
            // Center Rings
            pjRoutine.driveToTrajectory(3, -3, 135, false, 0, 1, slowConstraints)->run();
            intakeOnce(conveyor, pjRoutine);
            ...
            pjRoutine.driveToTrajectory(-11, 0, 180, false, 0, 1, slowConstraints)->run();
            intakeOnce(conveyor, pjRoutine);
            pjRoutine.driveToTrajectory(3, -3, 135, true, 0, 1, slowConstraints)->run();
            pjRoutine.driveToTrajectory(0, 11, 90, false, 0, 1, slowConstraints)->run();
            intakeOnce(conveyor, pjRoutine);
            pjRoutine.driveToTrajectory(3, -3, 135, true, 0, 1, slowConstraints)->run();

            // Mogo
            mogoGrabber.setMogoGrabbed(false);
            pjRoutine.driveToTrajectory(30, -40, 115, true, 0, 12)->run();
            pjRoutine.driveToTrajectory(24, -24, 135, true, 0, 2)->run();
            pjRoutine.rotateTo(235)->run();
            pjRoutine.driveToTrajectory(48, 0, 225, true, 0, 2)->run();
            mogoGrabber.setMogoGrabbed(true);
            ...
            pjRoutine.driveToTrajectory(54, -54, 315, false, 0, 2, slowConstraints)->run();
            intakeOnce(conveyor, pjRoutine);
            intake.setArmPosition(IntakeSystem::ArmPosition::NEUTRAL_STAKE);
            pjRoutine.pause(500)->run();
            pjRoutine.driveToTrajectory(60, -60, 315, false, 0, 2, fastConstraints)->run();
            intakeOnce(conveyor, pjRoutine);
            ...
            AutoBuilder &pjRoutine)
        {
            ...
            auto pauseConveyorStep = new AsyncPauseConveyorStep(conveyor, 1000);
